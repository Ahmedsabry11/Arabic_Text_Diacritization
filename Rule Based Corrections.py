def primary_diacritics_corrections(predicted_diacritized_string):
    ...
    ARABIC_LETTERS = ARABIC_LETTERS.union({' ', '0'})
    corrected_string = list(predicted_diacritized_string)
    print(corrected_string)
    i=0
    while i < len(predicted_diacritized_string):
        ...
        if corrected_string[i] == 'ا' and not((corrected_string[i+1]==' ') or (corrected_string[i+1] == 'َ' and corrected_string[i+1]==' ') or (corrected_string[i+1] == 'ً' and corrected_string[i+1]==' ')) :
            while corrected_string[i+1] not in ARABIC_LETTERS:
                i+=1
        ...
        if i+1 < len(corrected_string) and corrected_string[i] not in ['ء','ة'] and corrected_string[i+1] == 'ً':
            while corrected_string[i+1] not in ARABIC_LETTERS:
                corrected_string.pop(i+1)
            # print(corrected_string)
            # print("here7")

        if i+1 < len(corrected_string) and corrected_string[i] in ['0',' ','ى','آ','ا']:
            while corrected_string[i+1] not in ARABIC_LETTERS:
                corrected_string.pop(i+1)
            # print(corrected_string)
            # print("here8")
